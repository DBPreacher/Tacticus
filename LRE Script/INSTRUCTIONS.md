...
**Machines of War note:** Rows with `Is_MoW = Y` are Machines of War (Galatian, Reanimator, Tson'ji, Exorcist, Storm Speeder, Malleus Rocket Launcher, Rukkatrukk, Biovore, Forgefiend, Plagueburst Crawler, Z'kar). Most of these have no base on-battlefield stat block — they act purely through off-board abilities — so `Melee_Hits`, `Ranged_Hits`, `X_Hits_Restriction`, and sometimes the damage-type columns are left blank where the wiki gives no confirmed value. **Machines of War are never eligible for Legendary Event team-building, full stop** — `le_analysis.py`'s `analyse_track()` unconditionally drops every `Is_MoW=Y` row from that track's eligible pool (no per-event override in the yaml). An earlier version of this note said eligibility "depends on the event's specific rules" — that was wrong for LE mode and has been corrected; if a future event type genuinely does allow MoW, that would need new code (a yaml flag the exclusion checks), not just different judgment calls per analysis.

Z'kar is a partial exception to the "no base stat block" pattern — it has real `Melee_Hits` (3) and normal trait data, since it fields as a playable character before its ability ("When Z'kar is defeated or leaves the battlefield ... he returns as a Machine of War") triggers the transformation. It was originally miscategorized as `Is_MoW=N` for this reason and slipped into the LE 15 Lysander analysis's Beta Team 2 pool before being caught and corrected (see Changelog) — if another character's kit similarly blurs "playable character" and "Machine of War," don't assume a populated stat block rules out `Is_MoW=Y`; check the ability text and wiki classification directly.
...
Unlike `Melee_Damage_Type` and `Ranged_Damage_Type` (which only capture the character's *primary* weapon damage types and are reference text that `le_analysis.py` never reads), a `Has_[DamageType]` column is `Y` if the character deals that damage type from **any** source — primary melee/ranged attack **or** an active/passive ability. A character can have multiple damage types marked `Y` (e.g. a character with a Piercing melee attack whose ability triggers a Plasma-damage effect has both `Has_Piercing=Y` and `Has_Plasma=Y`).
...
**Relics never count — for any column.** Relics are optional equipment, not part of a character's base kit, so a relic's effect must never set a CSV value. This covers every column: traits (e.g. Snotflogga's Supa-Cybork Body relic makes him Mechanical, but `Mechanical=N`), `Has_[DamageType]`, `Self_Heal`, `Shielding`, `Spawner`, hits and damage types. Only the character's infobox traits, weapons, and active/passive abilities count. A value that relic text also mentions is still fine if the character's own kit provides it (e.g. Tyrith's passive deals Bolter damage, so `Has_Bolter=Y` even though her Paragon Spear relic does too). tacticustable.com shows an equipped relic in its `ITEMS` section and wiki.gg character pages link to theirs, so skip both when reading a kit. Relic effects are listed on the wiki's `Category:Relics` pages if you need to check whether something came from one.

This rule was confirmed by the owner in September 2026, replacing an earlier instruction to count an equipped relic's text. It changed three rows: Maugan Ra `Has_Direct` → N (Maugetar), and Corrodius and Nauseous Rotbone `Has_Toxic` → N (Orbs of Decay). The 1.42 patch notes confirm Rotbone only meets Toxic requirements when he has his relic equipped.
...
Machine of War wiki pages sometimes list traits that belong to the NPCs the MoW deploys, not the MoW itself (e.g. Malleus Rocket Launcher's "Battle Fatigue", Biovore's Flying and Instinctive Behaviour). Don't add those to the CSV. They're listed in `CONFIRMED` in `wiki_audit.py` so they aren't reported.
